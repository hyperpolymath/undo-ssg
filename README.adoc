...
"Flight" — The trajectory of reversible logic. A generator that moves forward to build and backwards to un-build.
...
Unlike traditional SSGs that "lose" source data during rendering, `flight-ssg` is built in a reversible language. Every build step is an isomorphism; the HTML output contains the mathematical potential to reconstruct the source markdown exactly.

=== Zero-Entropy Logic
Reversible operations avoid the energy cost of erasing information. `flight-ssg` is designed for high-efficiency hardware and **Edge tech** deployments where heat dissipation is a failure state.
...
== Part of poly-ssg
`flight-ssg` is the reversible, zero-entropy member of the **poly-ssg** family.
